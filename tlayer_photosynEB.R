...
# MODIFYING tleaf_photosynEB_2 to account for LAI depth
...
# Modified to calculate radiation at LAI depth
#### PPFD and Wind are top-of-canopy values
LeafEB_LAI <- function (Tleaf = 21.5, Tair = 20, gs = 0.15, PPFD = 1500, VPD = 2, 
  ...
  # LAI = LAI above this point
  # kd = extinction coefficient (for diffuse radiation)
  # kw = extinction coefficient for wind
...
# NB in canopy needs to be multiplied by absorptance kd exp(-kd*LAI)
# Leuning eqn D7
Gradiation <- 4 * Boltz * Tk(Tair)^3 * Emissivity/(CPAIR * AIRMA) * kd*exp(-kd*LAI)
# Boundary layer conductance for heat from forced convection
# Corrected for decline in wind speed with depth
Gbhforced <- 0.003 * sqrt(Wind*exp(-kw*LAI)/Wleaf) * CMOLAR 
# Grashof number
...
# Modified for LAI
Rsol <- (LeafAbsPAR + LeafAbsNIR) * PPFD/UMOLPERJ * kd*exp(-kd*LAI) # J m-2 s-1 - absorbed SW radiation = 2 * APAR
...
# See Leuning eqn D1
# Modified for LAI
Rnetiso <- Rsol - (Emissivity - ema) * Boltz * Tk(Tair)^4 * kd*exp(-kd*LAI)
